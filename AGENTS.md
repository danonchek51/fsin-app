...
- Нельзя обходить эту защиту: без того же явного решения владельца нельзя менять `migration.state: ready`, поля `continuity.*` или continuity evidence, чтобы переинициализировать среду либо сменить remote.
